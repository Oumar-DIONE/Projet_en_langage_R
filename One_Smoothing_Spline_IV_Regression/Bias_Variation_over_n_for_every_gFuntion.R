...
enableJIT(1)
k<-8
N <- c(400) #10*rep(1,k) #c(400,400)
cas<-3
Cases <- rep(cas,k) #1:8   #c(2,3)
Rhouv <-   seq(0.5,0.8,length=k)#c(.8,.8)
Rhoxw <- seq(0.7,0.9,length=k)  #c(.7,.7)
kernel <- "laplace"
...
for(j in 1:length(N)){
  print(paste("debut de l iteration numero ",j))
  ...
  res <- foreach(k = 1:gnrep, .combine='rbind', .errorhandling = 'remove') %dorng%{
    ...
    #ste2 Compute our estimator for every lambda
    obj <- rep(0,ll)
    for (j in (1:ll)){
      l <- lambda[j]
      giv<- rep(0,n)
    ...
    resut<-cbind(c(ghativ,lcviv,df.iv,0), c(phihat.eval.tikh, lambda.star,df.tikh, 0),
                 c(ghat.horow.eval, nk.horow, df.horow,0) , c(ghativ.prime,lcviv,df.iv,0) ) 
    
  }
  print(paste("fin de l iteration numero ",j))
  toc()
  stopCluster(cl)
...
ghativ <- as.matrix(ghativ)  #ajouter
meiv <- apply(ghativ,1,mean)
ghattik <- as.matrix(ghattik)  #ajouter
metik <- apply(ghattik,1,mean)
ghatgal <- as.matrix(ghatgal)  #ajouter
megal <- apply(ghatgal,1,mean)
meiv.prime <- apply(as.matrix(ghativ.prime),1,mean)
...
mvariv.prime <- mean(apply(as.matrix(ghativ.prime),2,function(a) mean((a-meiv.prime)^2)))
...
mseiv.prime <- mean(apply(as.matrix(ghativ.prime),2, function(a) mean((a - fx.prime)^2)))
...
madiv.prime <- mean(apply(as.matrix(ghativ.prime),2, function(a) mean(abs(a - fx.prime))))
...
Ac8biais_n_equal_100<-c(biasiv=biasiv,biastik=biastik,biasgal=biasgal)

Ac8biais_n_equal_400<-as.matrix(Ac8biais_n_equal_100)



write.csv(Ac8biais_n_equal_400, "Ac8biais_n_equal_400.csv", row.names = FALSE)
